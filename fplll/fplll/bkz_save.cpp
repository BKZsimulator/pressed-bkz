...
BKZReduction<ZT, FT>::BKZReduction(MatGSO<ZT, FT> &m, LLLReduction<ZT, FT> &lll_obj,
  ...
  : status(RED_SUCCESS), nodes(0), param(param), m(m), head(0), record_tour(0), lll_obj(lll_obj), algorithm(NULL),
      cputime_start(0)
{
                                ...
                                int max_row)
{
  ...
  if (par.flags & BKZ_DUMP_GSO)
	...
	{
	  if (record_tour >= 3999) // each iteration run 400 tours
	    {
	      cerr<<"record_tour heat 399" << record_tour << endl;
	      head ++;
	      rerandomize_block(head, max_row, 1);
	      record_tour = 0;
	    ...
	    {
	      record_tour ++;
	      //cerr << "record_tour" << record_tour;
	    }
	}
      clean &= trunc_tour(kappa_max, par, head, max_row);
      clean &= hkz(kappa_max, par, max(max_row - par.block_size, 0), max_row);
    }
  else
    {
      //cerr << "I am starting from " << min_row << endl;
      clean &= trunc_tour(kappa_max, par, min_row, max_row);
      clean &= hkz(kappa_max, par, max(max_row - par.block_size, 0), max_row);
    }

  // each iteration run 1 tours
  /*
  if (par.flags & BKZ_DUMP_GSO)
    {
      if (head >= m.d - par.block_size)
