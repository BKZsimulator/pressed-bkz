...
bool BKZReduction<ZT, FT>::svp_preprocessing(int kappa, int block_size, const BKZParam &param)
{
  ...
  int lll_start = (param.flags & BKZ_BOUNDED_LLL) ? kappa : head;
                                ...
                                int max_row)
{
  ...
  if (par.flags & BKZ_DUMP_GSO)
	...
	{
	  if (record_tour >= param.iter_tours) // freeze from 0 to head, and start from head+1
	    {
	      head ++;
	      flag_dumgso = true;
	  ...
	  else // continue on [head, head+bs-1]
	    {
	      record_tour ++;
	    }
	}
